export default function UnderwaterWaves() {
    return (
      <div className="absolute inset-0 w-full h-full">
        <div className="absolute w-full h-full">
          {/* First wave layer */}
          <div className="absolute w-[200%] h-full" style={{ top: '10%' }}>
            <div className="absolute w-full h-[40px] animate-wave-slow" 
              style={{
                background: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 1200 120\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M321.39 56.44c58-10.79 114.16-30.13 172-41.86 82.39-16.72 168.19-17.73 250.45-.39C823.78 31 906.67 72 985.66 92.83c70.05 18.48 146.53 26.09 214.34 3V0H0v27.35a600.21 600.21 0 00321.39 29.09z\'/%3E%3C/svg%3E")',
                backgroundRepeat: 'repeat-x',
                opacity: 0.2
              }}
            />
          </div>
  
          {/* Second wave layer */}
          <div className="absolute w-[200%] h-full" style={{ top: '20%' }}>
            <div className="absolute w-full h-[40px] animate-wave-medium" 
              style={{
                background: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 1200 120\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M321.39 56.44c58-10.79 114.16-30.13 172-41.86 82.39-16.72 168.19-17.73 250.45-.39C823.78 31 906.67 72 985.66 92.83c70.05 18.48 146.53 26.09 214.34 3V0H0v27.35a600.21 600.21 0 00321.39 29.09z\'/%3E%3C/svg%3E")',
                backgroundRepeat: 'repeat-x',
                opacity: 0.3
              }}
            />
          </div>
  
          {/* Third wave layer */}
          <div className="absolute w-[200%] h-full" style={{ top: '30%' }}>
            <div className="absolute w-full h-[40px] animate-wave-fast" 
              style={{
                background: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 1200 120\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M321.39 56.44c58-10.79 114.16-30.13 172-41.86 82.39-16.72 168.19-17.73 250.45-.39C823.78 31 906.67 72 985.66 92.83c70.05 18.48 146.53 26.09 214.34 3V0H0v27.35a600.21 600.21 0 00321.39 29.09z\'/%3E%3C/svg%3E")',
                backgroundRepeat: 'repeat-x',
                opacity: 0.4
              }}
            />
          </div>
        </div>
      </div>
    )
  }
  
  